...
public class BurpExtender implements IBurpExtender, IScannerCheck, IHttpListener, IExtensionStateListener {
    ...
    private static String basicInformationOutput() {
        ...
        String str4 = "author: Rainyseason-c\n";
        String str5 = "QQ: 2831258873\n";
        String str6 = "GitHub: https://github.com/Rainyseason-c\n";
        String str7 = "===================================\n";
    ...
    public void processHttpMessage(int toolFlag, boolean messageIsRequest, IHttpRequestResponse messageInfo) {
        if (messageIsRequest || messageInfo == null || messageInfo.getResponse() == null) {
            return;
        }

        try {
            if (this.isInternalProbeRequest(messageInfo)) {
                this.addDebugToUi(messageInfo, "[debug] HTTP listener skip internal probe response", "", "");
                return;
            }

            CustomBurpUrl burpUrl = new CustomBurpUrl(this.callbacks, messageInfo);
            String url = burpUrl.getHttpRequestUrl().toString();
            String statusCode = this.helpers.analyzeResponse(messageInfo.getResponse()).getStatusCode() + "";

            this.addDebugToUi(
                    messageInfo,
                    "[debug] HTTP listener received response, tool=" + this.callbacks.getToolName(toolFlag),
                    statusCode,
                    url
            );

            // Newer Burp versions may not invoke the legacy IScannerCheck#doPassiveScan path
            // for normal proxy traffic. Run the same passive scan from the HTTP listener as a
            // fallback, then manually add produced issues into Burp Scanner.
            if (!this.httpListenerScanningUrls.add(url)) {
                this.addDebugToUi(messageInfo, "[debug] HTTP listener fallback skip duplicate/in-progress url", statusCode, url);
                return;
            }

            try {
                this.addDebugToUi(messageInfo, "[debug] HTTP listener fallback scan start", statusCode, url);
                List<IScanIssue> issues = this.doPassiveScan(messageInfo);
                if (issues == null || issues.size() == 0) {
                    this.addDebugToUi(messageInfo, "[debug] HTTP listener fallback scan finished: no issue", statusCode, url);
                    return;
                }

                int addedIssueNumber = 0;
                ...
                this.addDebugToUi(messageInfo, "[debug] HTTP listener fallback scan finished: added issues=" + addedIssueNumber, statusCode, url);
            } finally {
                this.httpListenerScanningUrls.remove(url);
            }
        } catch (Exception e) {
    ...
    public List<IScanIssue> doPassiveScan(IHttpRequestResponse baseRequestResponse) {
        ...
        if (domainNameBlacklist != null && domainNameBlacklist.size() >= 1) {
            if (isMatchDomainName(baseBurpUrl.getRequestHost(), domainNameBlacklist)) {
                this.addDebugToUi(baseRequestResponse, "[debug] skip: domain blacklist matched", this.helpers.analyzeResponse(baseRequestResponse.getResponse()).getStatusCode() + "", baseBurpUrl.getHttpRequestUrl().toString());
                return null;
            }
        }
        ...
        if (domainNameWhitelist != null && domainNameWhitelist.size() >= 1) {
            if (!isMatchDomainName(baseBurpUrl.getRequestHost(), domainNameWhitelist)) {
                this.addDebugToUi(baseRequestResponse, "[debug] skip: domain whitelist not matched", this.helpers.analyzeResponse(baseRequestResponse.getResponse()).getStatusCode() + "", baseBurpUrl.getHttpRequestUrl().toString());
                return null;
            }
        }
        ...
        if (this.isUrlBlackListSuffix(baseBurpUrl)) {
            this.addDebugToUi(baseRequestResponse, "[debug] skip: url suffix blacklist matched", this.helpers.analyzeResponse(baseRequestResponse.getResponse()).getStatusCode() + "", baseBurpUrl.getHttpRequestUrl().toString());
            return null;
        }
        ...
        if (!shiroFingerprint.run().isRunExtension()) {
            this.addDebugToUi(baseRequestResponse, "[debug] fingerprint result: no runnable fingerprint module", this.helpers.analyzeResponse(baseRequestResponse.getResponse()).getStatusCode() + "", baseBurpUrl.getHttpRequestUrl().toString());
            this.tags.getScanQueueTagClass().add(
                    "",
                    "",
                    this.helpers.analyzeRequest(baseRequestResponse).getMethod(),
        ...
        if (!shiroFingerprint.run().isShiroFingerprint()) {
            this.addDebugToUi(shiroFingerprint.run().getHttpRequestResponse(), "[debug] fingerprint result: not shiro, method=" + shiroFingerprint.run().getExtensionName(), this.helpers.analyzeResponse(shiroFingerprint.run().getHttpRequestResponse().getResponse()).getStatusCode() + "", baseBurpUrl.getHttpRequestUrl().toString());
            if (messageLevel.equals("ALL")) {
                this.tags.getScanQueueTagClass().add(
                        "",
                        "",
        ...
        this.addDebugToUi(shiroFingerprint.run().getHttpRequestResponse(), "[debug] fingerprint result: shiro found, method=" + shiroFingerprint.run().getExtensionName(), this.helpers.analyzeResponse(shiroFingerprint.run().getHttpRequestResponse().getResponse()).getStatusCode() + "", baseBurpUrl.getHttpRequestUrl().toString());
        ...
        try {
            ...
            this.stdout.println(String.format("[Shiro] scan finished | %s", httpRequestUrl));
        } catch (Exception e) {
            ...
            this.stdout.println(String.format("[Shiro] scan error | %s | see Errors tab", baseBurpUrl.getHttpRequestUrl().toString()));
    ...
    private boolean isInternalProbeRequest(IHttpRequestResponse requestResponse) {
        try {
            for (String header : this.helpers.analyzeRequest(requestResponse).getHeaders()) {
                if (header.toLowerCase().startsWith(CustomBurpHelpers.INTERNAL_PROBE_HEADER.toLowerCase() + ":")) {
                    return true;
                }
            }
    ...
    private boolean isDebugShowInUi() {
        try {
            Object value = this.yamlReader.getValueByKey("debug.showInUi");
    ...
    private void addDebugToUi(IHttpRequestResponse requestResponse, String issue, String statusCode, String url) {
        if (!this.isDebugShowInUi()) {
            return;
        }
        if (requestResponse == null || this.tags == null || this.tags.getScanQueueTagClass() == null) {
            return;
        }
        try {
            String method = "";
            try {
                method = this.helpers.analyzeRequest(requestResponse).getMethod();
            } catch (Exception ignored) {
            }
            this.tags.getScanQueueTagClass().add(
                    "debug",
